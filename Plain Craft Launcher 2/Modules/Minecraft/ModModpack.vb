    ...
    Public Sub ModpackInstall()
        ...
        Sub()
            ...
            Catch ex As Exception
                Log(ex, "手动安装整合包失败", NotifyLevel.MsgBox)
            End Try
        End Sub)
    End Sub
    ''' <summary>
...
#Region "不同类型整合包的安装方法"
    ...
    Private Function InstallPackCurseForge(FileAddress As String, Archive As Compression.ZipArchive, ArchiveBaseFolder As String,
        ...
        If ModList.Any Then
            ...
            Sub(Task As LoaderTask(Of Integer, JArray))
                ...
                Log("[Modpack] 缺失的 fileIds：" & MissingIds.Join("、"))
        ...
        Sub(Task As LoaderTask(Of String, String))
            ...
            For Each Target As String In {VersionFolder & "原始整合包.zip", VersionFolder & "原始整合包.mrpack"}
                ...
                End If
            Next
            If File.Exists(FileAddress) AndAlso Path.GetFileNameWithoutExtension(FileAddress) = "modpack" Then
    ...
    Private Function InstallPackModrinth(FileAddress As String, Archive As Compression.ZipArchive, ArchiveBaseFolder As String, Optional InstanceName As String = Nothing, Optional Logo As String = Nothing) As LoaderCombo(Of String)
        ...
        For Each File In If(Json("files"), {})
            ...
            If Not Path.GetFullPath(TargetPath).StartsWithF($"{McFolderSelected}versions\{InstanceName}\", True) Then
        ...
        Sub(Task As LoaderTask(Of String, Integer))
            ...
            Try
                ...
                If File.Exists(MmcSetupFile) Then
                    ...
                    For Each Line In ReadFile(MmcSetupFile).Split(vbCrLf.ToCharArray, StringSplitOptions.RemoveEmptyEntries)
