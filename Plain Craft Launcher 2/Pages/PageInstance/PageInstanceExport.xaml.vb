...
#Region "配置文件"
    ...
    Private Sub ExportConfig() Handles BtnAdvancedExport.Click
        Try
            ...
            Hint(GetLang("LangPageVersionExportHintSavedExportConfig", ConfigPath), HintType.Green)
    ...
    Private Sub ImportConfig() Handles BtnAdvancedImport.Click
        Try
            ...
            End If
            '结束
            Hint(GetLang("LangPageVersionExportHintReadExportConfig", ConfigPath), HintType.Green)
        Catch ex As Exception
        ...
        If Not String.IsNullOrWhiteSpace(ConfigPackPath) AndAlso
           (Not ConfigPackPath.EndsWithF("\") AndAlso Not ConfigPackPath.EndsWithF("/")) Then
            ...
            Catch ex As Exception
                ...
                If MyMsgBox(GetLang("LangPageVersionExportDialogContentConfigInvalid", ConfigPackPath, ex.GetDetail), GetLang("LangPageVersionExportDialogTitleConfigInvalid"), GetLang("LangDialogBtnOK"), GetLang("LangDialogBtnCancel")) = 2 Then Return
            End Try
        End If
        If PackPath Is Nothing Then
            Dim Extensions As New List(Of String)
            Extensions.Add(GetLang("LangPageVersionExportSelectExportPathSaveAsFormat", "zip", "zip"))
            If Not CheckOptionsPcl.Checked Then Extensions.Add(GetLang("LangPageVersionExportSelectExportPathSaveAsFormat", "mrpack", "mrpack"))
        ...
        Sub(Loader As LoaderTask(Of Integer, List(Of McMod)))
            ...
            For Each Line In AllExtraFiles
                If Line.EndsWithF("\") OrElse Line.EndsWithF("/") Then
                    ...
                    Else
                        Hint(GetLang("LangPageVersionExportHintFolderInConfigNotFound", Line), HintType.Red)
                    End If
                Else
                    If File.Exists(Line) Then
                        CopyFile(Line, BaseFolder & GetFileNameFromPath(Line))
                    Else
                        Hint(GetLang("LangPageVersionExportHintFileInConfigNotFound", Line), HintType.Red)
                    End If
                End If
            Next
